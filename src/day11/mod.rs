...
pub fn part01(input: &str) -> i64 {
    let (no_galaxy_rows, no_galaxy_cols, points) = parse_universe(input);
    let mut res = 0;
...
fn parse_universe(input: &str) -> (Vec<usize>, Vec<usize>, Vec<(usize, usize)>) {
    ...
    (no_galaxy_rows, no_galaxy_cols, points)
}
...
pub fn part02(input: &str) -> i64 {
    ...
    for i in 0..points.len() {
        for j in i+1..points.len() {
            ...
            let extra_moves = (get_extra_moves(p1, p2, &no_galaxy_rows, &no_galaxy_cols) *999999 ) as i64;
            let normal_moves =
                (p1.0 as i64 - p2.0 as i64).abs() + (p1.1 as i64 - p2.1 as i64).abs();
            // println!(
            //     "{:?}-{:?} extra:{:?},total:{:?}",
